...
const roleOptions = [{ label: '全部', value: '' }, { label: '管理员', value: 'ADMIN' }, { label: '普通用户', value: 'USER' }];
...
const columns: any[] = [
  ...
  { title: '角色', key: 'role', render: (r: any) => h(NTag, { type: r.role === 'ADMIN' ? 'error' : 'info', size: 'small' }, () => r.role === 'ADMIN' ? '管理员' : '用户') },
    ...
    <NModal v-model:show="showModal" preset="card" title="编辑用户" style="width:600px">
      <NForm :model="form" label-placement="left" label-width="80px">
        <NGrid :cols="2" :x-gap="16">
          ...
          <NGridItem><NFormItem label="角色"><NSelect v-model:value="form.role" :options="[{label:'管理员',value:'ADMIN'},{label:'用户',value:'USER'}]" /></NFormItem></NGridItem>
